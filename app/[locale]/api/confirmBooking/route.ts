...
import { generateFreePhotoPDF, generateBookingSummaryPDF } from "@/lib/email";
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < adults; i++) {
      const buffer = await generateBookingSummaryPDF(booking);
      pdfBuffers.push({
    ...
    for (let i = 0; i < children; i++) {
      const buffer = await generateFreePhotoPDF(booking);
      pdfBuffers.push({
